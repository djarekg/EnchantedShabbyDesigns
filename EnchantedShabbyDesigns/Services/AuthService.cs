...
public sealed class AuthService : IAuthService
{
    ...
    public async Task<IResult<Domain.Auth>> AddAsync(AuthModel model)
    {
        var validation = new AuthModelValidator().Validation(model);

        if (validation.Failed) return validation.Fail<Domain.Auth>();

        if (await _authRepository.AnyByLoginAsync(model.Login)) return Result<Domain.Auth>.Fail("Login exists!");
    ...
    private IResult<TokenModel> CreateToken(Domain.Auth auth)
    {
